...
class ZhihuProvider(BaseProvider):
            ...
            async with aiohttp.ClientSession() as session:
                ...
                async with session.get(self.hot_url, headers=headers, timeout=aiohttp.ClientTimeout(total=10)) as response:
                    if response.status == 200:
                        ...
                        logger.error(f"Failed to fetch from Zhihu, status code: {response.status}")
                        return []
    ...
    def parse(self, html: str) -> List[Dict]:
        ...
        try:
            ...
            # 查找热榜列表
            hot_list = soup.find('div', class_='HotList')
            if not hot_list:
                return []

            items = hot_list.find_all('div', class_='HotItem')
            for item in items:
                try:
                    ...
                    logger.error(f"Error parsing Zhihu item: {str(e)}")
                    continue
    ...
    def _parse_item(self, item) -> Dict:
        ...
        # 获取热度
        hot_score_elem = item.find('div', class_='HotItem-metrics')
        hot_score = 0
        if hot_score_elem:
        ...
        # 获取摘要
        content_elem = item.find('p', class_='HotItem-excerpt')
        content = content_elem.get_text(strip=True) if content_elem else ''

        # 获取作者/来源
        author_elem = item.find('div', class_='HotItem-meta')
